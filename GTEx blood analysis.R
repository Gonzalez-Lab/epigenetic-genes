...
age.decade = ifelse(samples.df$age.decade=="20-29",25,
...
EpiGenes_expr <- samples.df[,-c(1:12)]
rownames(EpiGenes_expr) <- rownames(samples.df)

keep <- colMeans(EpiGenes_expr > 1) > 0.2
EpiGenes_expr2 <- EpiGenes_expr[, keep]
EpiGenes_expr2[] <- lapply(EpiGenes_expr2, as.numeric)

EpiGenes_expr_log <- log2(EpiGenes_expr2+1) #samples en filas, genes en columnas

EpiGenes_expr_log2 <- t(EpiGenes_expr_log) #genes en filas, samples en columnas

samples.matrix <- cbind(EpiGenes_expr_log,age)

PC <- prcomp(EpiGenes_expr_log, scale = TRUE)
PC$rotation
summary(PC)

cor.test(PC$x[,1], age.decade, method="spearman")
cor.test(PC$x[,2], age.decade, method="spearman") #captura senial temporal
cor.test(PC$x[,3], age.decade, method="spearman") #captura senial temporal
cor.test(PC$x[,4], age.decade, method="spearman")

summary(lm(age.decade ~ PC$x[,2] + PC$x[,3]))
